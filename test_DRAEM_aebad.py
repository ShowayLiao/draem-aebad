...
from data_loader import MVTecDRAEMTestDataset,AeBADDRAEMTestDataset
...
def test(obj_names, mvtec_path, checkpoint_path, base_model_name):
    ...
    for obj_name in obj_names:
        ...
        # flops_1,paras_1 = profile(model, inputs=(dummy_input,))
        # print("flops 1 {}, paras 1 {}".format(flops_1, paras_1))
        # del dummy_input
        ...
        # dummy_input_2 = torch.randn(1, 6, img_dim, img_dim).cuda()
        # flops_2, paras_2 = profile(model_seg, inputs=(dummy_input_2,))
        # print("flops 2 {}, paras 2 {}".format(flops_2, paras_2))
        # del dummy_input_2



        # dataset = MVTecDRAEMTestDataset(mvtec_path + obj_name + "/test/", resize_shape=[img_dim, img_dim])
        dataset = AeBADDRAEMTestDataset(mvtec_path, resize_shape=[img_dim, img_dim],obj_name=obj_name)
        ...
        with torch.no_grad():
            for i_batch, sample_batched in enumerate(dataloader):
                ...
                is_normal = sample_batched["has_anomaly"].detach().numpy()[0]
                ...
                # print("Reconstruction time: ", start.elapsed_time(end), "ms")
                ...
                # end.record()
                # torch.cuda.synchronize()
                # print("Segmentation time: ", start.elapsed_time(end), "ms")
                out_mask_sm = torch.softmax(out_mask, dim=1)

                del joined_in,out_mask
